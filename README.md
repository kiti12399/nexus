...
Telegram-бот для продажи API-ключей: каталог товаров, склад ключей, резервирование заказа и выдача ключа после подтверждения оплаты.
...
- Склад API-ключей по каждому товару.
- Шифрование ключей в базе через Fernet.
- Резерв ключа при создании заказа.
- Оплата криптовалютой по указанному кошельку.
- Уведомление админа, когда пользователь нажал "Я оплатил".
- Ручное подтверждение заказа админом и автоматическая отправка ключа пользователю.
- Команды для товаров, склада и заказов.
- Веб-кабинет: регистрация, вход, профиль, телефон, Telegram-привязка, заказы и ключи.

Подробная схема деплоя: [DEPLOYMENT.md](DEPLOYMENT.md).

Используйте только ключи, которые вы имеете право продавать или выдавать пользователям. Для цифровых товаров внутри Telegram также проверьте актуальные правила Telegram и требования платежных провайдеров.
...
```text
/add_product gpt 490 GPT API Key
...
`/approve_order` отправит пользователю купленный API-ключ и пометит ключ проданным.
...
Заказы и выданные ключи из Telegram подтягиваются в кабинет после привязки Telegram.
...
Следующий шаг: сделать сайт-чекаут, который будет создавать заказ аккаунта, принимать оплату и после подтверждения добавлять купленный ключ в личный кабинет.
